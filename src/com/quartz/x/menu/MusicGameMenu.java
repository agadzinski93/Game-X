...
public class MusicGameMenu extends ShipMenu
{
	...
	public MusicGameMenu(Player player)
	{
		super(player);
		try
		{
			...
			up = new MainButton(new Rectangle(425 + STATS_WIDTH / 2, 440, 170, 74), "UP");
			down = new MainButton(new Rectangle(675 + STATS_WIDTH / 2, 440, 170, 74), "DOWN");
			
			titleOptions = new MenuLabel("MUSIC", 580 + STATS_WIDTH / 2, 230);
			volumeText = new MenuLabel("VOLUME: ", 540 + STATS_WIDTH / 2, 410);
			exitClose = new MenuLabel("X", 912 + STATS_WIDTH / 2, 190);
	...
	public void update(Point pointPressed, Point pointReleased)
	{
		...
		if (released)
		{
			if (pointReleased.getX() >= 912 + STATS_WIDTH / 2 && pointReleased.getX() <= 932 + STATS_WIDTH / 2 && pointReleased.getY() >= 170 && pointReleased.getY() <= 190)
			{
				State.setState(STATE.OPTIONS_GAME);
				Sound.click();
			}
			else if (pointReleased.getX() >= 515 + STATS_WIDTH / 2 && pointReleased.getX() <= 585 + STATS_WIDTH / 2 && pointReleased.getY() >= 305 && pointReleased.getY() <= 345)
			{
				if (isMute())
				{
					Menu.setMute(false);
			...
			else if (pointReleased.getX() + STATS_WIDTH / 2 >= 705 && pointReleased.getX() <= 800 + STATS_WIDTH / 2 && pointReleased.getY() >= 305 && pointReleased.getY() <= 345)
			{
				if (!isMute())
				{
					Menu.setMute(true);
	...
	public void render(Graphics g)
	{
		...
		g.drawImage(soundSwitch, 450 + STATS_WIDTH / 2, 300, soundSwitchWidth, soundSwitchHeight, null);
		up.render(g);
		down.render(g);
